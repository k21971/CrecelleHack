...
cursed_book(struct obj *bp)
{
    ...
    case 5:
        ...
        if (uarmg) {
            erode_obj(uarmg, "gloves", ERODE_CORRODE, EF_GREASE | EF_VERBOSE | EF_DESTROY);
            break;
        }
        /* temp disable in_use; death should not destroy the book */
        was_in_use = bp->in_use;
        bp->in_use = FALSE;
        poison_strdmg(resist_reduce(rn1(4, 3), POISON_RES) + rn1(2, 1),
                      resist_reduce(rnd(6), POISON_RES) + rnd(6),
...
deadbook(struct obj *book2)
{
 ...
 raise_dead:
        ...
        mm.x = u.ux;
        mm.y = u.uy;
        mkundead((struct monst *) NULL, &mm, TRUE, NO_MINVENT);
