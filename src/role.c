...
const struct Role roles[NUM_ROLES+1] = {
    ...
    { { "Grappler", 0 },
      { { "Jobber", 0 },
        ...
        { "Babyface", 0 },
        { "Champ", 0 } },
      "Hugo", "King", "_Vice", /* Famous grappler names */
      "Grp",
      "the Training Centre",
      "the Great Ring",
      PM_GRAPPLER,
...
const struct Race races[NUM_RACES + 1] = {
    ...
    },
    {
        ...
        PM_ORC_MUMMY,
        PM_ORC_ZOMBIE,
        MH_ORC | ROLE_MALE | ROLE_FEMALE | ROLE_LAWFUL | ROLE_NEUTRAL
            | ROLE_CHAOTIC,
        MH_ORC,
        0,
...
ok_race(int rolenum, int racenum, int gendnum, int alignnum)
{
    ...
    if (IndexOkT(racenum, races)) {
        ...
        for (i = 0; i < SIZE(races) - 1; i++) {
            ...
            allow = races[i].allow;
            if (IndexOkT(rolenum, roles)
                && !(roles[rolenum].mhrace & races[i].selfmask))
                continue;
            if (gendnum >= 0 && gendnum < ROLE_GENDERS
                && !(allow & genders[gendnum].allow & ROLE_GENDMASK))
                continue;
...
void
clearrolefilter(int which UNUSED)
{
    int i;
