...
NHSUBDIR="crecellehack-1.6.0"
# VAR_PLAYGROUND from include/unixconf.h
NH_VAR_PLAYGROUND="/crecellehack-1.6.0/var/"
# END OF CONFIG
##############################################################################
...
#   - every running binary holds crecellehack/nhdat open for the whole session
...
mkdir -p "$NAO_CHROOT/dgldir/inprogress-cre160"
chown "$USRGRP" "$NAO_CHROOT/dgldir/inprogress-cre160"
...
if [ -n "$NETHACKBIN" -a -e "$NETHACKBIN" ]; then
  ...
  # in that window finds no `crecellehack` at all.
  ...
  if [ -n "$_cur" ] && [ -e "$_cur" ] && cmp -s "$NETHACKBIN" "$_cur"; then
    echo "  crecellehack unchanged ($_cur) - not installing a new dated binary"
  else
    cp "$NETHACKBIN" "$NHBINFILE"
    chown root:root "$NHBINFILE"
    chmod 755 "$NHBINFILE"
    ln -sfn "$NHBINFILE" "crecellehack.new.$$"
    mv -T "crecellehack.new.$$" crecellehack
    echo "  crecellehack -> $NHBINFILE"
  fi
...
install_atomic "$NETHACK_GIT/dat/NHdump.css" "$NAO_CHROOT/$NHSUBDIR/NHdump.css" 644
